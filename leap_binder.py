...
from dataset_selection.cifar10_resnet.utils import metadata_animal, metadata_fly, metadata_label_name, metadata_gt_label
from dataset_selection.cifar10_resnet.preprocess import preprocess_func
from dataset_selection.cifar10_resnet.encoders import input_encoder
from dataset_selection.cifar10_resnet.config import CONFIG


# Preprocess Function
def preprocess_func_leap() -> List[PreprocessResponse]:
    train_X, val_X, train_Y, val_Y, test_x, test_y = preprocess_func()

    # Generate a PreprocessResponse for each data slice, to later be read by the encoders.
    # The length of each data slice is provided, along with the data dictionary.
    train = PreprocessResponse(length=train_X.shape[0],
                               data={'images': train_X, 'labels': train_Y, 'subset_name': 'train'})
    val = PreprocessResponse(length=val_X.shape[0],
                             data={'images': val_X, 'labels': val_Y, 'subset_name': 'val'})
    test = PreprocessResponse(length=test_x.shape[0],
                              data={'images': test_x, 'labels': test_y, 'subset_name': 'test'})
